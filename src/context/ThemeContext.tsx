...
const ThemeProvider: React.FC<ThemeProviderProps> = ({ children }) => {
  ...
  const variants = {
    ...
    "--link-color-hover": adjustLightness(getAccessibleTextColor(backgroundColor), -20),
  ...
  if (process.env.NODE_ENV === "development") {
    ...
    console.log("  Primary:", contrastRatio(primaryColor, variants["--text-color-primary"]).toFixed(2));
    console.log("  Secondary:", contrastRatio(secondaryColor, variants["--text-color-secondary"]).toFixed(2));
    console.log("  Tertiary:", contrastRatio(tertiaryColor, variants["--text-color-tertiary"]).toFixed(2));
    console.log("  Quaternary:", contrastRatio(quaternaryColor, variants["--text-color-quaternary"]).toFixed(2));
  }
}, [selectedScheme]);
