...
import ChipGroup, { ChipItem } from "../components/Chip/ChipGroup/ChipGroup";
...
describe("ChipGroup", () => {
  ...
  it("auto-closes chips after duration", () => {
    ...
    act(() => {
      jest.advanceTimersByTime(2300); // 2000ms + 300ms fade
    });
  ...
  it("applies chip position and stackIndex styles", () => {
    render(<ChipGroup chips={defaultChips} position="bottom-left" />);
    const chip = screen.getByText("Chip One").parentElement;
    expect(chip?.className).toMatch(/bottom-left/);
  });
});
